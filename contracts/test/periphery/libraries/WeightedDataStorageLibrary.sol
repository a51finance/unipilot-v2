...
library WeightedDataStorageLibrary {
    ...
    function consult(address pool, uint32 period)
        internal
        view
        returns (PeriodTimepoint memory timepoint)
    {
        require(period != 0, "BP");
        ...
        timepoint.harmonicMeanLiquidity = uint128(
    ...
    function getArithmeticMeanTickWeightedByLiquidity(
        ...
        for (uint256 i; i < timepoints.length; i++) {
            numerator +=
                int256(timepoints[i].harmonicMeanLiquidity) *
                timepoints[i].arithmeticMeanTick;
