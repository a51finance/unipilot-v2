...
library PriceMovementMath {
    ...
    {
        function(uint160, uint160, uint128)
            pure
            returns (uint256) getAmountA = zeroToOne
                ? getTokenADelta01
                : getTokenADelta10;
        ...
        if (amountAvailable >= 0) {
            ...
            if (amountAvailableAfterFee >= input) {
                ...
                resultPrice = getNewPriceAfterInput(
                    currentPrice,
                    liquidity,
            ...
            output = (zeroToOne ? getTokenBDelta01 : getTokenBDelta10)(
                resultPrice,
                currentPrice,
                liquidity
            );
            ...
            function(uint160, uint160, uint128)
                pure
                returns (uint256) getAmountB = zeroToOne
                    ? getTokenBDelta01
                    : getTokenBDelta10;
            ...
            if (uint256(amountAvailable) >= output) resultPrice = targetPrice;
                ...
                resultPrice = getNewPriceAfterOutput(
                    currentPrice,
                    liquidity,
                    uint256(amountAvailable),
                    zeroToOne
