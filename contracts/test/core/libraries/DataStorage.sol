...
library DataStorage {
    ...
    function _volatilityOnRange(
        ...
        volatility = uint256(
            (K**2 * sumOfSquares + 6 * B * K * sumOfSequence + 6 * dt * B**2) /
                (6 * dt**2)
        );
    }
    ...
    function createNewTimepoint(
        ...
        last.secondsPerLiquidityCumulative += ((uint160(delta) << 128) /
            (liquidity > 0 ? liquidity : 1)); // just timedelta if liquidity == 0
    ...
    function binarySearch(
        ...
        private
        view
        returns (Timepoint storage beforeOrAt, Timepoint storage atOrAfter)
    {
        uint256 left = oldestIndex; // oldest timepoint
        uint256 right = lastIndex >= oldestIndex
            ? lastIndex
            : lastIndex + UINT16_MODULO; // newest timepoint considering one index overflow
        ...
        do {
            ...
            (bool initializedBefore, uint32 timestampBefore) = (
            ...
            if (initializedBefore) {
                if (lteConsideringOverflow(timestampBefore, target, time)) {
                    ...
                    (bool initializedAfter, uint32 timestampAfter) = (
                        atOrAfter.initialized,
                        atOrAfter.blockTimestamp
                    );
                    if (initializedAfter) {
                        if (
                            lteConsideringOverflow(target, timestampAfter, time)
                        ) {
    ...
    function getSingleTimepoint(
        ...
        if (
            secondsAgo == 0 ||
            lteConsideringOverflow(self[index].blockTimestamp, target, time)
        ) {
        ...
        (
            Timepoint memory beforeOrAt,
            Timepoint memory atOrAfter
        ) = binarySearch(self, time, target, index, oldestIndex);
        ...
        if (target != beforeOrAt.blockTimestamp) {
            // we're in the middle
            uint32 timepointTimeDelta = atOrAfter.blockTimestamp -
                beforeOrAt.blockTimestamp;
        ...
        returns (uint88 volatilityAverage, uint256 volumePerLiqAverage)
    {
        ...
        Timepoint memory endOfWindow = getSingleTimepoint(
            self,
            time,
            0,
            tick,
        ...
        if (lteConsideringOverflow(oldestTimestamp, time - WINDOW, time)) {
            ...
            uint144 _oldestVolumePerLiquidityCumulative = oldest
                .volumePerLiquidityCumulative;
            return (
                (endOfWindow.volatilityCumulative -
                    _oldestVolatilityCumulative) / (time - oldestTimestamp),
                uint256(
                    endOfWindow.volumePerLiquidityCumulative -
