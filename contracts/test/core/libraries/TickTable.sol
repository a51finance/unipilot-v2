...
library TickTable {
    ...
    function toggleTick(mapping(int16 => uint256) storage self, int24 tick)
        internal
    {
    ...
    function getSingleSignificantBit(uint256 word)
        internal
        pure
    ...
    function getMostSignificantBit(uint256 word)
        internal
        pure
    ...
    ) internal view returns (int24 nextTick, bool initialized) {
        {
            ...
            assembly {
                tick := sub(
                    ...
                    and(slt(tick, 0), not(iszero(smod(tick, tickSpacing))))
                )
            }
        }
    ...
    function uncompressAndBoundTick(int24 tick)
        private
        pure
        returns (int24 boundedTick)
    {
