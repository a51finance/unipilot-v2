...
contract TickTableTest {
    ...
    function nextTickInTheSameRow(int24 tick, bool lte)
        external
        view
    ...
    function getGasCostOfNextTickInTheSameRow(int24 tick, bool lte)
        external
        view
        returns (uint256)
    {
    ...
    function isInitialized(int24 tick) external view returns (bool) {
        (int24 next, bool initialized) = bitmap.nextTickInTheSameRow(
            tick,
            true
        );
