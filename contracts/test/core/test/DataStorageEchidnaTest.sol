...
contract DataStorageEchidnaTest {
    ...
    function initialize(
        ...
        dataStorage.initialize(
            DataStorageTest.InitializeParams({
                time: time,
                tick: tick,
                liquidity: liquidity
            })
    ...
    function checkTimeWeightedResultAssertions(
        ...
        uint256 timeWeightedHarmonicMeanLiquidity = (uint256(timeElapsed) *
            type(uint160).max) /
            (uint256(
                secondsPerLiquidityCumulatives[1] -
    ...
    function echidna_canAlwaysGetPoints0IfInitialized()
        external
        view
        returns (bool)
    {
    ...
    function checkVolatilityOnRangeNotOverflowUint88(
        ...
        uint256 res = dataStorage.volatilityOnRange(
            dt,
            tick0,
            tick1,
            avgTick0,
    ...
    function checkTwoAdjacentTimepointsTickCumulativeModTimeElapsedAlways0(
        uint16 index
    ) external view {
        ...
        ) = dataStorage.timepoints(index == 0 ? 65536 - 1 : index - 1);
        (
            bool initialized1,
    ...
    function checkTimeWeightedAveragesAlwaysFitsType(uint32 secondsAgo)
        external
        view
    {
        require(initialized);
