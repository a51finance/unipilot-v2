...
contract AlgebraPool is PoolState, PoolImmutables, IAlgebraPool {
        ...
        )
    {
        ...
        (int24 currentTick, uint16 currentTimepointIndex) = (
            globalState.tick,
        ...
        if (liquidityDelta == 0) {
            ...
            if (liquidityDelta < 0) {
                ...
                if (_liquidityCooldown > 0) {
                    require(
                        (_blockTimestamp() - lastLiquidityAddTimestamp) >=
                            _liquidityCooldown
                    );
                }
            }
            ...
            uint128 liquidityNext = LiquidityMath.addDelta(
                currentLiquidity,
                liquidityDelta
            );
        ...
        )
    {
        ...
        (uint256 _totalFeeGrowth0Token, uint256 _totalFeeGrowth1Token) = (
        ...
        )
    {
        ...
        uint256 receivedAmount1;
        {
            ...
            IAlgebraMintCallback(msg.sender).algebraMintCallback(
                amount0,
                amount1,
                data
            );
            if (amount0 > 0)
                require(
                    (receivedAmount0 = balanceToken0() - receivedAmount0) > 0,
                    "IIAM"
                );
            if (amount1 > 0)
                require(
                    (receivedAmount1 = balanceToken1() - receivedAmount1) > 0,
                    "IIAM"
                );
        }
        ...
        if (receivedAmount1 < amount1) {
            uint128 liquidityForRA1 = uint128(
                FullMath.mulDiv(
                    uint256(liquidityActual),
                    receivedAmount1,
                    amount1
...

        {
            ...
            require(
                (amount0 = uint256(amount0Int)) <= receivedAmount0,
                "IIAM2"
            );
            require(
                (amount1 = uint256(amount1Int)) <= receivedAmount1,
                "IIAM2"
            );
        }
    ...
    function collect(
        ...
        Position storage position = getOrCreatePosition(
            msg.sender,
            bottomTick,
            topTick
        );
        ...
        amount0 = amount0Requested > positionFees0
            ? positionFees0
            : amount0Requested;
    ...
    function _getNewFee(
        ...
        newFee = IDataStorageOperator(dataStorageOperator).getFee(
            _time,
            _tick,
            _index,
            _liquidity
    ...
    function _swapCallback(
        ...
        IAlgebraSwapCallback(msg.sender).algebraSwapCallback(
            amount0,
            amount1,
            data
        );
    ...
    function swap(
        ...
        (
            ...
            currentLiquidity,
            communityFee
        ) = _calculateSwapAndLock(zeroToOne, amountRequired, limitSqrtPrice);
        ...
        if (zeroToOne) {
            ...
            require(
                balance0Before.add(uint256(amount0)) <= balanceToken0(),
                "IIA"
            );
        } else {
            if (amount0 < 0)
            ...
            _swapCallback(amount0, amount1, data); // callback to get tokens from the caller
            require(
                balance1Before.add(uint256(amount1)) <= balanceToken1(),
                "IIA"
            );
        }
    ...
    function swapSupportingFeeOnInputTokens(
        ...
        if (zeroToOne) {
            ...
            require(
                (amountRequired = int256(balanceToken0().sub(balance0Before))) >
                    0,
                "IIA"
            );
        } else {
            ...
            require(
                (amountRequired = int256(balanceToken1().sub(balance1Before))) >
                    0,
                "IIA"
            );
        }
        ...
        if (zeroToOne) {
            ...
            if (amount0 < amountRequired)
                TransferHelper.safeTransfer(
                    token0,
                    sender,
                    uint256(amountRequired.sub(amount0))
                );
        } else {
            if (amount0 < 0)
                TransferHelper.safeTransfer(
            ...
            if (amount1 < amountRequired)
                TransferHelper.safeTransfer(
                    token1,
                    sender,
                    uint256(amountRequired.sub(amount1))
                );
        }
        ...
        )
    {
        ...
        SwapCalculationCache memory cache;
        {
            ...
            if (activeIncentive != address(0)) {
                IAlgebraVirtualPool.Status _status = IAlgebraVirtualPool(
                    activeIncentive
                ...
                if (_status == IAlgebraVirtualPool.Status.NOT_EXIST) {
                    ...
                    cache.incentiveStatus = IAlgebraVirtualPool
                        .Status
                        .NOT_STARTED;
                }
            }
        ...
        while (true) {
            ...
            if (cache.exactInput) {
                ...
                cache.amountCalculated = cache.amountCalculated.sub(
                    step.output.toInt256()
                ); // decrease calculated output amount
            } else {
                amountRequired += step.output.toInt256(); // increase remaining output amount (since its negative)
                cache.amountCalculated = cache.amountCalculated.add(
                    (step.input + step.feeAmount).toInt256()
            ...
            if (currentPrice == step.nextTickPrice) {
                ...
                if (step.initialized) {
                    ...
                    if (
                        cache.incentiveStatus !=
                        IAlgebraVirtualPool.Status.NOT_EXIST
                    ) {
                        IAlgebraVirtualPool(activeIncentive).cross(
                            step.nextTick,
                            zeroToOne
                        );
                    }
        ...
        (liquidity, volumePerLiquidityInBlock) = (
            currentLiquidity,
            cache.volumePerLiquidityInBlock +
                IDataStorageOperator(dataStorageOperator)
    ...
    function flash(
        ...
        IAlgebraFlashCallback(msg.sender).algebraFlashCallback(
            fee0,
            fee1,
            data
        );
        ...
        if (paid0 > 0) {
            ...
            if (_communityFeeToken0 > 0) {
                fees0 =
                    (paid0 * _communityFeeToken0) /
                    Constants.COMMUNITY_FEE_DENOMINATOR;
                TransferHelper.safeTransfer(token0, vault, fees0);
            }
            totalFeeGrowth0Token += FullMath.mulDiv(
                paid0 - fees0,
                Constants.Q128,
                _liquidity
            );
        ...
        if (paid1 > 0) {
            ...
            if (_communityFeeToken1 > 0) {
                fees1 =
                    (paid1 * _communityFeeToken1) /
                    Constants.COMMUNITY_FEE_DENOMINATOR;
                TransferHelper.safeTransfer(token1, vault, fees1);
            }
            totalFeeGrowth1Token += FullMath.mulDiv(
                paid1 - fees1,
                Constants.Q128,
                _liquidity
            );
    ...
    function setCommunityFee(uint8 communityFee0, uint8 communityFee1)
        external
        override
        lock
        onlyFactoryOwner
    {
        require(
            (communityFee0 <= Constants.MAX_COMMUNITY_FEE) &&
                (communityFee1 <= Constants.MAX_COMMUNITY_FEE)
        );
        (globalState.communityFeeToken0, globalState.communityFeeToken1) = (
            communityFee0,
            communityFee1
        );
    ...
    /// @inheritdoc IAlgebraPoolPermissionedActions
    function setLiquidityCooldown(uint32 newLiquidityCooldown)
        external
        override
        onlyFactoryOwner
    {
        require(
            newLiquidityCooldown <= Constants.MAX_LIQUIDITY_COOLDOWN &&
