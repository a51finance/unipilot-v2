...
import "../../v3-lm-pool/contracts/interfaces/IPancakeV3LmPool.sol";
...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(
            msg.sender == factory ||
                msg.sender == IPancakeV3Factory(factory).owner()
        );
        _;
    }
    ...
    function swap(
        ...
        SwapCache memory cache = SwapCache({
            ...
            feeProtocol: zeroForOne
                ? (slot0Start.feeProtocol % PROTOCOL_FEE_SP)
        ...
        while (
            ...
            if (cache.feeProtocol > 0) {
                uint256 delta = (step.feeAmount.mul(cache.feeProtocol)) /
    ...
    function flash(
        ...
        if (paid0 > 0) {
            ...
            uint256 fees0 = feeProtocol0 == 0
                ? 0
                : (paid0 * feeProtocol0) / PROTOCOL_FEE_DENOMINATOR;
        ...
        if (paid1 > 0) {
            ...
            uint256 fees1 = feeProtocol1 == 0
                ? 0
                : (paid1 * feeProtocol1) / PROTOCOL_FEE_DENOMINATOR;
    ...
    function setFeeProtocol(uint32 feeProtocol0, uint32 feeProtocol1)
        external
        override
        lock
        onlyFactoryOrFactoryOwner
    {
        require(
            (feeProtocol0 == 0 ||
                (feeProtocol0 >= 1000 && feeProtocol0 <= 4000)) &&
                (feeProtocol1 == 0 ||
                    (feeProtocol1 >= 1000 && feeProtocol1 <= 4000))
        );
    ...
    function setLmPool(address _lmPool)
        external
        override
        onlyFactoryOrFactoryOwner
    {
