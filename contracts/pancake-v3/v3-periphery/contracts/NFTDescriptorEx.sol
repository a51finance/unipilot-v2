...
import "../../v3-core/contracts/interfaces/IPancakeV3Pool.sol";
...
contract NFTDescriptorEx {
    ...
    event ToggleSwitchAndUpdateNFTDomain(
        address indexed sender,
        bool switchToHttpLink,
        string NFTDomain
    );
        ...
        returns (string memory)
    {
        ...
        return
            switchToHttpLink
                ? string(
                    abi.encodePacked(bytes(NFTDomain), "/", bytes(tokenUri))
                )
                : tokenUri;
    }
    ...
    function toggleSwitchAndUpdateNFTDomain(
        bool _switchToHttpLink,
        string memory _NFTDomain
    ) external onlyOwner {
        switchToHttpLink = _switchToHttpLink;
        NFTDomain = _NFTDomain;

        emit ToggleSwitchAndUpdateNFTDomain(
            msg.sender,
            _switchToHttpLink,
            _NFTDomain
        );
